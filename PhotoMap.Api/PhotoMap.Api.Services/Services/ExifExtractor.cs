...
namespace PhotoMap.Api.Services.Services
{
    public class ExifExtractor : IExifExtractor
    {
        ...
        {
            return subIfd.TryGetByte(tag, out var byteValue) ? byteValue : null;
        }

        private static int? ParseInt(Directory directory, int tag)
        {
            return directory.TryGetInt32(tag, out var intValue) ? intValue : null;
        }
        ...
        private static DateTime? ParseDateTime(string dateTimeStr, DateTimeStyles dateTimeStyles)
        {
            ...
            if (DateTime.TryParseExact(dateTimeStr, new[] { dateTimeFormat1, dateTimeFormat2 },
                    CultureInfo.InvariantCulture, dateTimeStyles, out var dateTime))
            {
                return dateTime;
            }
